...
class NistSpider(scrapy.Spider):
    ...
    #start_urls = ['https://webbook.nist.gov/cgi/formula/Ac',]
            ...
            if  tmp!=None and tmp.find("Other data")!=-1:
                ...
                for each in eachs:
                    ...
                    if temp!=None and temp.find("ion energetics")!=-1:
                        ...
                        url=prefix+each.xpath("./a/@href").extract_first()
                        yield scrapy.Request(url,callback=self.data_parse)
    
    def data_parse(self, response):
        data=[]
        y=Nist()
        y['Name']=response.xpath('//*[@id="Top"]')
        items= response.xpath('//*[@id="main"]//ul//li')
        for no,item in enumerate(items):
            tmp=item.xpat("./strong/text()").extract_first()
            if no==0:
                y['Formula']=item.xpath("./text()")
            if tmp!=None and tmp.find("CAS")!=-1:
                y['CAS']=item.xpath("./text()")
